...
def make_atlas_api_request(method, url, auth, params=None, json_data=None):
    ...
    try:
        response = requests.request(method, url, auth=auth, params=params, json=json_data, headers=headers, timeout=30)
...
def get_all_paginated_items(url, auth, item_key="results", max_pages=100):
    ...
    while True:
        ...
        if current_page > max_pages: # Safety break
            logger.warning(f"Reached max_pages ({max_pages}) limit for {url}. Not all items might be fetched.")
            break
    return all_items
...
def delete_atlas_database_user(project_id, username, auth, db_name="admin"):
    ...
    logger.info(f"  Attempting to delete database user: {username} from project {project_id}...")
    ...
    if result is None: # Successful deletion returns 204 No Content
        ...
        logger.error(f"  Failed to delete database user: {username}. Response: {result}")
        return False
...
def delete_atlas_project_user(project_id, user_id, auth):
    ...
    logger.info(f"  Attempting to remove Atlas user ID: {user_id} from project {project_id}...")
    result = make_atlas_api_request("DELETE", url, auth)
    if result is None: # Successful removal returns 204 No Content
        logger.info(f"  Successfully removed Atlas user ID: {user_id} from project {project_id}.")
        return True
    else:
...
def delete_atlas_cluster(project_id, cluster_name, auth):
    ...
    logger.info(f"  Attempting to delete cluster: {cluster_name} from project {project_id}...")
    result = make_atlas_api_request("DELETE", url, auth)
    if result is None: # Successful deletion returns 204 No Content
...
def delete_invitations_for_old_projects(org_id, org_invitations, old_project_ids, auth):
    ...
    logger.info(f"  Found old projects requiring cleanup. Deleting ALL {len(org_invitations)} pending organization invitations...")
    ...
    for invitation in org_invitations:
        ...
        if invitation_id:
            logger.info(f"  Deleting invitation for user: {username} (ID: {invitation_id})")
            if delete_atlas_org_invitation(org_id, invitation_id, auth):
                successful += 1
            else:
                failed += 1
        else:
            logger.warning(f"  Skipping invitation with missing ID for user: {username}")
            failed += 1
    
    logger.info(f"  Invitation cleanup completed: {successful} successful, {failed} failed")
    return successful, failed
...
def delete_atlas_org_invitation(org_id, invitation_id, auth):
    ...
    logger.info(f"  Attempting to delete invitation ID: {invitation_id} from organization...")
    result = make_atlas_api_request("DELETE", url, auth)
    if result is None: # Successful deletion returns 204 No Content
        logger.info(f"  Successfully deleted invitation ID: {invitation_id}")
        return True
    else:
        logger.error(f"  Failed to delete invitation ID: {invitation_id}. Response: {result}")
        return False
...
def main():
    ...
    try:
        ...
        print("Additionally:")
        print("  - ALL pending organization invitations will be deleted (if any old projects are found)")
        ...
        logger.info(f"Found {len(org_invitations)} total invitations at organization level")

        user_deletion_threshold_utc = CURRENT_DATE_UTC - timedelta(days=USER_DELETION_THRESHOLD)
        cluster_deletion_threshold_utc = CURRENT_DATE_UTC - timedelta(days=CLUSTER_DELETION_THRESHOLD)
        logger.info(f"User deletion threshold: {user_deletion_threshold_utc.strftime('%Y-%m-%dT%H:%M:%SZ')}")
        logger.info(f"Cluster deletion threshold: {cluster_deletion_threshold_utc.strftime('%Y-%m-%dT%H:%M:%SZ')}")
        ...
        for project in projects:
            ...
            try:
                ...
                logger.error(f"Could not parse creation date ('{created_str}') for project {project_name} ({project_id}). Skipping.")
                total_errors += 1
                continue
            ...
            if created_date < user_deletion_threshold_utc:
                logger.info(f"Project {project_name} ({project_id}) is older than {USER_DELETION_THRESHOLD} days. Proceeding with user cleanup.")
                total_cleaned += 1
                ...
                if db_users:
                    logger.info(f"  Found {len(db_users)} database users to process for project {project_name}")
                    for db_user in db_users:
                        ...
                        if username in ["__onprem_monitoring", "admin"]:
                            ...
                            continue
                        if username:
                            delete_atlas_database_user(project_id, username, auth, db_user.get("databaseName", "admin"))
                else:
                ...
                if atlas_users_in_project:
                    logger.info(f"  Found {len(atlas_users_in_project)} Atlas users with access to project {project_name}")
                    ...
                    logger.info(f"  No Atlas users found with access to project {project_name}")

                # 3. Delete Clusters if project is older than 120 days
                if created_date < cluster_deletion_threshold_utc:
                    logger.info(f"  Project {project_name} is older than {CLUSTER_DELETION_THRESHOLD} days. Proceeding with cluster deletion.")
                    clusters = get_atlas_clusters(project_id, auth)
                    if clusters:
                        logger.info(f"  Found {len(clusters)} clusters to delete in project {project_name}")
                    ...
                    logger.info(f"  Project {project_name} is not older than {CLUSTER_DELETION_THRESHOLD} days. Skipping cluster deletion.")

            else:
                logger.info(f"Project {project_name} ({project_id}) is not older than {USER_DELETION_THRESHOLD} days. Skipping user cleanup.")
        ...
        if old_project_ids:
            ...
            successful_inv, failed_inv = delete_invitations_for_old_projects(org_id, org_invitations, old_project_ids, auth)
            logger.info(f"Organization invitations cleanup: {successful_inv} successful, {failed_inv} failed")
        else:
            logger.info("\nNo old projects found, skipping organization invitation cleanup")

        logger.info(f"Reaper script completed. Projects processed: {total_processed}, Cleaned: {total_cleaned}, Errors: {total_errors}")
