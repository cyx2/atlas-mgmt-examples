...
# Email addresses to invite - modify this list as needed
EMAILS_TO_PROVISION = [
    "example+adfb89d8fs@mongodb.com",
    "example+afdfafrege@mongodb.com",
]
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/invite_users_to_org.log"), logging.StreamHandler()],
)
...
def invite_users_to_org(org_id: str, emails: List[str]) -> bool:
    ...
    for email in emails:
        ...
        response = make_atlas_api_request("POST", url, json=payload, headers=headers, auth=auth)
    ...
    logger.info(f"Invitation process completed. Successful: {successful_invites}, Failed: {failed_invites}")
...
def main():
    ...
    try:
        ...
        print(f"About to invite {len(EMAILS_TO_PROVISION)} users to organization {ORGANIZATION_ID}")
